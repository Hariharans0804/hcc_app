...
const DistributorAgentTodayPaidAmountList = () => {
    ...
    const fetchTodayPaidDistributorList = async () => {
        try {
            ...
            const paidAmountDetails = response.data.filter(
                (item) =>
                    item.type === 'paid' &&
                    item.colldate === currentDate &&
                    item.agent_id === agentloginUserData
            );
    ...
    useFocusEffect(
        useCallback(() => {
            const fetchData = async () => {
                setLoading(true);
                await Promise.all([fetchEmployeesData(), fetchTodayPaidDistributorList(), fetchGetAgentLoginUserData()]);
                setLoading(false);
            };
            fetchData();
    ...
    const searchUniqueDistributors = todayPaidDistributorAmount.filter((item) => {
    ...
    const today_Local_Total_Payments = searchUniqueDistributors.reduce(
...
export default DistributorAgentTodayPaidAmountList
